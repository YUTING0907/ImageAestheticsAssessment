...
from torch_aesthetics.losses import RegRankLoss,EDMLoss
from torch_aesthetics.models import RegressionNetwork
from torch_aesthetics.aadb import AADB, load_transforms,load_transforms_flip
...
def create_data_part(cfg: DictConfig):
    ...
    train_dataset = train_dataset+train_dataset_aug
    # Setup dataloaders
...
def train(model,opt,loss_fn,train_dataloader,writer,epoch,n_iter):
    ...
    for batch, (x, y,file_name) in pbar:
        opt.zero_grad()
        print("y:::",y)
...
def start_train(cfg):
    ...
    for epoch in range(cfg.train.epochs):
        ...
        filename = "{}_epoch_{}_srcc_{:.4f}_lcc_{:.4f}_loss_{:.4f}_.pt".format(
            cfg.data.dataset,epoch,vsrcc,vlcc, val_loss
        )
        #torch.save(model.state_dict(), os.path.join(cfg.train.save_dir, filename))
        torch.save(model, os.path.join(cfg.train.save_dir, filename))
...
def check(cfg):
    ...
    model = model.to(cfg.device).to(torch.float32)
    model.eval()
    model.load_state_dict(torch.load(cfg.data.model_path),cfg.device) 
    ...
    #        loss += F.mse_loss(y_pred, y)
            ...
            #if(file_name=="./data/aadb/images/farm1_393_20184676445_ea5b02f869_b.jpg"):
        ...
        for i in merged_list:
            if(i[0]=='./data/aadb/images/farm1_393_20184676445_ea5b02f869_b.jpg'):
                print(merged_list[i][0])
                print([round(num, 2) for num in merged_list[i][1]])
                print([round(num, 2) for num in merged_list[i][2]])
