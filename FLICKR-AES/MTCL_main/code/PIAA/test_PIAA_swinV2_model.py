...
from models.train_FLICKR_swinv2_model import *
from torchvision import models
from models.train_FlickrAES_Contrast_swinV2_model import Contrast_model
...
class FlickrAESDataset_PIAA(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        sample = {'image': image, 'rating': rating, 'imagename': img_name}
...
class Rescale(object):
    ...
    def __call__(self, sample):
        ...
        return {'image': image, 'rating': rating,'imagename':sample['imagename']}
...
def train_model(model, criterion, optimizer, dataloader_train, dataloader_valid,worker, num_epochs=100):
    since = time.time()
    best_spearman = 0
    criterion.cuda()
            ...
            if phase == 'val':
                ...
                for batch_idx, data in enumerate(dataloader_valid):
                    ...
                    with open("output.txt", "w") as file:
                        file.write(f"worker:{worker} image_name:{data['imagename']} pred: {outputs_a},true: {labels}\n")
                    ratings.append(labels)
                    predictions.append(outputs_a)
...
def Flicker_test():
    data_dir = os.path.join('/home/ps/temp/model/aesthetic2/MTCL_main/code/PIAA/FlickrAES_PIAA/label')

    workers_test = pd.read_csv(os.path.join(data_dir, 'test_workers_ID.csv'), sep=' ')
    ...
    for worker_idx in range(37):
        ...
        worker = workers_test['ID'].unique()[worker_idx]
        ...
        for i in range(0, 10):
            ...
            transformed_dataset_train = FlickrAESDataset_PIAA(csv_file=train_path,
                                                            root_dir='/home/ps/temp/model/FLICKR-AES/image',
            ...
            giaa_model = torch.load('/home/ps/temp/model/aesthetic2/MTCL_main/code/model/swin-transV2/swin-transV2-FlickrAes-TANet-GIAA.pt')
            #giaa_model.load_state_dict(
            #    torch.load('./code/model/ResNet-50/ResNet50-FlickrAes-GIAA.pt')
            #)
            contrast_model = Contrast_model(giaa_model.backbone)
            contrast_model = torch.load('/home/ps/temp/model/aesthetic2/MTCL_main/code/model/swin-transV2/swin-transV2-FlickrAes-Contrast.pt')
            #contrast_model.load_state_dict(
            #    torch.load('./code./model/ResNet-50/ResNet50-FlickrAes-Contrast.pt'))
            ...
            spearman = train_model(model_ft, criterion, optimizer, dataloader_train,
                                   dataloader_valid,worker,num_epochs=epochs)
            srocc_list.append(spearman)
        srocc = np.mean(srocc_list)
        print("-------average srocc is: %4f-------" % srocc)
        spp.append(srocc)
        torch.save(model_ft.cuda(), '../model/swin-transV2/PIAA_%s.pt' % worker)
    print(spp)
    print(np.mean(spp))
    torch.save(model_ft.cuda(), '../model/swin-transV2/PIAA.pt')
