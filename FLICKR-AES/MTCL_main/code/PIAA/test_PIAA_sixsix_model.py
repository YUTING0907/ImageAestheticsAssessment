...
from models.train_FLICKR_sixsixsix_model import GIAA_model
from torchvision import models
from models.train_FlickrAES_Contrast_sixsix_model import *
...
class PIAA_model(nn.Module):
    def __init__(self, Contrast_model, GIAA_model):
        ...
        self.GIAA_model = GIAA_model
        self.tahn = nn.Tanh()
        for param in self.GIAA_model.parameters():
            param.requires_grad = False
        self.regression = nn.Sequential(nn.Linear(2048, 1), nn.Sigmoid())
    ...
    def forward(self, x):
        ...
        #print("GIAA_feature:",GIAA_feature.shape)
        Contrast_feature = self.Contrast_model_encoder(x)
        #print("Constrast_feature:",Contrast_feature.shape)
...
def Flicker_test():
    ...
    for worker_idx in range(37):
        ...
        for i in range(0, 10):
            ...
            contrast_model = torch.load('/home/ps/temp/model/aesthetic2/MTCL_main/code/model/ResNet-50/ResNet50-FlickrAes-sixsixContrast.pt')
            #contrast_model.load_state_dict(
            #    torch.load('./code./model/ResNet-50/ResNet50-FlickrAes-Contrast.pt'))
            '''
            giaa = GIAAModel()
            
            contrast_model = Contrast_model(giaa)
            piaa_model = PIAA_model(contrast_model, giaa)
