...
class AADBDataset_GIAA(Dataset):
    """Images dataset."""
    ...
    def __getitem__(self,item):
        row = self.images_frame.iloc[item]
        y = np.array([row['score']])
        image_name = row['image_name']
        image_path = os.path.join(self.root_dir, f'{image_name}')
        # 替换windows反斜杠
        image_path = image_path.replace('\\', '/')
        image = default_loader(image_path)
        #x = self.transform(image)
        balacing_elements = np.array([row['BalacingElements']])
        color_harmony = np.array([row['ColorHarmony']])
        content = np.array([row['Content']])
        doF = np.array([row['DoF']])
        light = np.array([row['Light']])
        motion_blur = np.array([row['MotionBlur']])
        object = np.array([row['Object']])
        repetition = np.array([row['Repetition']])
        rule_of_thirds = np.array([row['RuleOfThirds']])
        symmetry = np.array([row['Symmetry']])
        vivid_color = np.array([row['VividColor']])
        scores_names = ['score','BalacingElements','ColorHarmony','Content','DoF','Light','MotionBlur','Object','Repetition','RuleOfThirds','Symmetry','VividColor']
        #y=np.array([row[k] for k in scores_names])
        #return x,y.astype('float32')
        #print(image_name,y,balacing_elements,color_harmony,content,doF,light,motion_blur,object,repetition,rule_of_thirds,symmetry,vivid_color)
        #return x, y.astype('float32'),balacing_elements.astype('float32'),color_harmony.astype('float32'),content.astype('float32'),doF.astype('float32'),light.astype('float32'),motion_blur.astype('float32'),object.astype('float32'),repetition.astype('float32'),rule_of_thirds.astype('float32'),symmetry.astype('float32'),vivid_color.astype('float32')
        sample = {'image':image,'rating':y}
        if self.transform:
...
def load_data():
    data_dir = os.path.join(r'/home/ps/temp/model/AADB/dataset/csv')
    data_image_dir = os.path.join(r'/home/ps/temp/model/AADB/dataset/image')
    data_image_train_dir = os.path.join(data_dir, 'merged_train_validation.csv')
    data_image_test_dir = os.path.join(data_dir, 'merged_test.csv')

    transformed_dataset_train = AADBDataset_GIAA(
    ...
    transformed_dataset_valid = AADBDataset_GIAA(
