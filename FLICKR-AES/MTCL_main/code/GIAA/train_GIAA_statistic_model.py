...
class GIAA_model(nn.Module):
    def __init__(self, backbone, out_dim=2048):
        ...
        # create regression
        self.regression = nn.Sequential(nn.Linear(out_dim, 4), nn.Sigmoid())
...
class ToTensor(object):
    ...
    def __call__(self, sample):
        ...
        return {'image': torch.from_numpy(image).double(),
                'rating': torch.from_numpy(rating.values.astype('float64')).double()}
...
def load_data():
    ...
    data_image_train_dir = os.path.join(data_dir, 'train_with_statistics.csv')
    data_image_test_dir = os.path.join(data_dir, 'test_with_statistics.csv')
        ...
        for phase in ['train', 'valid']:
            ...
            if phase == 'valid':
                ...
                for batch_idx, data in enumerate(data_valid):
                    ...
                    predicts_score += outputs.tolist()[0]
                    #predicts_score += outputs.mean(axis=1).tolist()
                    ratings_score += labels.tolist()[0]
                    #ratings_score += labels.mean(axis=1).tolist()
                ...
                if srcc > BestSRCC:
                    ...
                    torch.save(best_model.cuda(), '../model/ResNet-50/ResNet50-FlickrAes-GIAA_statistic.pt')
